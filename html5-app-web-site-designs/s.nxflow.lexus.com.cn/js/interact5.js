$(function(){
    ...
    var scripts = {
        'part5': function(){
            ...
            var containerNode = $('.part5 .container');
            var viewNode = $('.viewport');
            var containerOffsetY;
            var bg = $('.part5 .bg');
            var bg2 = $('.part5 .bg2');
            var bg3 = $('.part5 .bg3');
            var txt_01 = $('.part5 .txt_01');
            var txt_01_2 = $('.part5 .txt_01_2');
            var txt_02 = $('.part5 .txt_02');
            var txt_03 = $('.part5 .txt_03');
            var txt_03_2 = $('.part5 .txt_03_2');
            var circle_tip = $('.part5 .circle_tip');
            var step = 0;
            var oriX;
            var oriY;
            var offsetX;
            ...
            function init(){
                ...
                TweenLite.to(viewNode,0.6,{opacity:1, ease:Power1.easeInOut, onComplete:function(){
                    magicShow(txt_01, function(){
                        magicShow(txt_01_2, function(){
                            TweenLite.set(circle_tip, {x:180, left: '0%', top:'74%', opacity:0, scale:1.1});
                        ...
                        txt_01_2.find('div div').each(function(index, node){
                            node = $(node);
                            if (node.html() == '一' && !isAndroid){
                                TweenLite.set(node, {position:'relative',top:'0.4em'});
                            }
                        });
                    }, 1.5);
                }});
                ...
                if (isAndroid){
                    txt_01.addClass('android');
                    txt_01_2.addClass('android');
                    txt_02.addClass('android');
                    txt_03.addClass('android');
                    txt_03_2.addClass('android');
                }else{
                    txt_01.addClass('ios');
                    txt_01_2.addClass('android');
                    txt_02.addClass('ios');
                    txt_03.addClass('android');
                    txt_03_2.addClass('ios');
                }
            ...
            function startHandle(e){
                ...
                switch (step){
                    case 1:
                        ...
                        TweenLite.to(txt_01_2, 0.5, {opacity:0});
                        TweenLite.set(bg2, {display:'block', opacity: 0});
                        TweenLite.to(bg2, 0.6, {opacity:1, onComplete: function(){
                            magicShow(txt_02, null, 1.5);
                            TweenLite.delayedCall(0, function moveCir(){
                                TweenLite.set(circle_tip, {opacity:0, x:0, left:'25%', top:'10%'});
                                TweenLite.fromTo(circle_tip, 0.8, {opacity:0}, {opacity:1, delay:1});
                                TweenLite.fromTo(circle_tip, 1.8, {left:'25%', top:'10%'}, {left:'25%', top:'60%', delay:1.8, ease:Power1.easeInOut});
                    ...
                    case 3:
                        ...
                        TweenLite.killTweensOf(circle_tip);
                        TweenLite.to(circle_tip, 0.5, {opacity:0});
                        oriX = e.originalEvent.touches[0].clientX / utils.globalScale;
                        oriY = e.originalEvent.touches[0].clientY / utils.globalScale - containerOffsetY;
            ...
            function moveHandle(e){
                if (step != 3) return;
                offsetX = e.originalEvent.touches[0].clientX / utils.globalScale - oriX;
                offsetY = e.originalEvent.touches[0].clientY / utils.globalScale - containerOffsetY - oriY;
                var dis = Math.sqrt(Math.pow(offsetX, 2) + Math.pow(offsetY, 2));
                if (dis >= gap){
                    var cate = Math.floor(Math.random()*4) + 2;
                    var line = $('<div class="line'+cate+'"></div>');
                    var angle = Math.atan2(offsetY, offsetX) / Math.PI * 180 + 90 + Math.random() * 20 - 10;
                    TweenLite.set(line, {x:oriX+offsetX+Math.random()*6-3, y:oriY+offsetY+Math.random()*6-3, rotationZ:angle});
                    containerNode.append(line);
                    oriX = e.originalEvent.touches[0].clientX / utils.globalScale;
                    oriY = e.originalEvent.touches[0].clientY / utils.globalScale - containerOffsetY;
                    lineCount ++;
                    gap = Math.random()*(gapMax-gapMin)+gapMin;
                    if (lineCount > maxCount){
                        step = 4;
                        TweenLite.to(txt_02, 0.5, {opacity:0});
                        magicShow(txt_03, function moveCir(){
                            magicShow(txt_03_2, function moveCir(){
                                step = 5;
                            }, 1.5);
                            txt_03_2.find('div div').each(function(index, node){
                                node = $(node);
                                if (node.html() == '一' && !isAndroid){
                                    TweenLite.set(node, {position:'relative',top:'0.4em'});
                                }
            ...
            function endHandle(e){
                ...
                if (lineCount > maxCount * 0.75 && step == 3){
                    step = 4;
                    TweenLite.to(txt_02, 0.5, {opacity:0});
                    magicShow(txt_03, function moveCir(){
                        magicShow(txt_03_2, function moveCir(){
...
//        TweenLite.to(this.wrap,1,{opacity:0,delay:1});
    ...
    function preload(cb) {
        ...
        imgLoader.addGroup({
            name: 'part5',
            imgArr: [
                'assets/demo2/part5/bg.jpg',
                'assets/demo2/part5/bg2.jpg',
                'assets/demo2/part5/line2.png',
                'assets/demo2/part5/line3.png',
                'assets/demo2/part5/line4.png',
                'assets/demo2/part5/line5.png',
            ]
        });
        imgLoader.loadGroup('part5');
